 ...
 */
#define DEFAULT_SLEEP_MAX_FREQ 304000
...
static void cpufreq_idle_start(void)
{
...
static void cpufreq_idle_end(void)
{
	struct savagedzen_info_s *this_savagedzen = &per_cpu(savagedzen_info,
smp_processor_id());
...
static int cpufreq_savagedzen_idle_notifier(struct notifier_block *nb,
					unsigned long val,
					void *data)
{
	switch (val) {
...
static struct notifier_block cpufreq_savagedzen_idle_nb = {
	.notifier_call = cpufreq_savagedzen_idle_notifier,
};
...
static int __init cpufreq_savagedzen_init(void)
{
	...
	idle_notifier_register(&cpufreq_savagedzen_idle_nb);
